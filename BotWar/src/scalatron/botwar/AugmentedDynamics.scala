...
//            if(x) println(s"player ${player.name} : ${player.cpuTime.doubleValue / 1e6} ms Reference: ${refCpu.doubleValue / 1e6} ms")
          ...
          thisVariety match {
            ...
            // case thisPlayer: Bot.Player => println(s"${thisPlayer.name} exceeded 50 ms, ${(thisPlayer.cpuTime.doubleValue / state.time / 1e6).formatted("%.1fms")}")
  ...
  def processCollision(movingBot: Bot, proposedPos: XY, steppedOnBot: Bot, state: State, board: Board): Board = {
    ...
    movingBot.variety match {
      case movingPlayer: Bot.Player =>
        ...
        def updateStateMap(extension: (String, String)) {
          updatedBoard = updatedBoard.updateBot(movingBot.updateVariety(movingPlayer.updatedStateMap(extension)))
        }

        /** Record the collision in the entity's state map */
        def bonk() {
          updateStateMap(Protocol.PropertyName.Collision, (proposedPos - movingBotPos).toString)
        }
