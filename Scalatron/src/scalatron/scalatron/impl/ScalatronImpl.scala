...
object ScalatronImpl {
  /** Creates an instance of a Scalatron server. The returned instance is also the main entry
    ...
    * @return
    */
  def apply(argMap: Map[String, String], actorSystem: ActorSystem, verbose: Boolean, stateListener: OutwardState => Unit): ScalatronImpl = {
    ...
    ScalatronImpl(
      ...
      pluginBaseDirectoryPath,
      TournamentState.withListeners(stateListener),
      secureMode,
      verbose
    )
  }
...
case class ScalatronImpl(
  ...
  def postStepCallback(mostRecentState: UntypedState) {
  ...
  def run(argMap: Map[String, String]) {
    val rounds = argMap.get("-rounds").fold(Int.MaxValue)(_.toInt)
    val headless = argMap.get("-headless").getOrElse("no") == "yes"
    val rebuild = argMap.get("-rebuild").getOrElse("no") == "yes"
    if (rebuild) {
      users().foreach {
        u =>
          println(s"rebuild for ${u.name}")
