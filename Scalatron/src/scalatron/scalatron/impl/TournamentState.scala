...
object TournamentState {
  ...
  def withListeners(stateListener: Simulation.OutwardState => Unit) = new TournamentState(stateListener)
}


class TournamentState(stateListener: Simulation.OutwardState => Unit) {
  var roundsPlayed = 0
